...
int NumberOfLayers = 7; // 607 is a million hexigon in a circle
int StartRuns = 0;
int NumberOfRuns = 1;
...
void cerialViralTransmission(int Nx, int Ny, int cell2cell, int freecell) {
        float te = Te(TauE,ne);
        float ti = Ti(TauI, ni);
            ...
            */
            ...
            while(timestepcount < (NumberofTimeSteps - 1)) {
                ...
                if (RUNCPU == 0) {
                    ...
                    // cerialViralTransmission(Nx, Ny, cell2cell, freecell);
