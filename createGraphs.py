...
##############################################################################
# change these parameters to match the parameters of the simulation you are graphing
# most of these values can be determined by looking at the "simulation parameters" section in the viral transmission simulation code
MOI = [2.0] # can be multiple values of MOI
NumberOfRuns = 1
OuterPath = r"~/path/to/ViralModel/" # 
NumberOfLayers = 607
regenParams = [0.003, 0.01, 0.03, 0.1, 0.3, 1.0, 3.0] # must be floats
cellToCell = False
##############################################################################
...
for j in range(len(MOI)):
    ...
    for k in regenParams:
        for i in range(NumberOfRuns):
            ...
            if cellToCell:
                InnerPath = r"/{}_0-CELL2CELL_{}-MOI_{}-RP".format(NumberOfLayers, MOI[j], k)
            else:
                InnerPath = r"/{}_0-FREECELL_{}-MOI_{}-RP".format(NumberOfLayers, MOI[j], k)
